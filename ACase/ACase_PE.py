...
ACase_y = torch.tensor(ACase_data, dtype=torch.float32)
...
output_file = 'ACase/Result/MVGP_result.csv'
...
checkpoint = torch.load('multitask_gp_checkpoint_ACase.pth', map_location=Device)
...
for row_idx in range(ACase_y.shape[0]):
    input_point = ACase_y[row_idx, :]
    ...
    estimated_params_tmp, _ = Estimation.multi_start_estimation(
        MVGP_models, MVGP_likelihoods, row_idx, ACase_y, bounds,
...
# nohup python ACase_PE.py > ACase_PEout.log 2>&1 &